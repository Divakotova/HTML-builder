...
async function createDirectory(directory) {
    await fsPromises.mkdir(path.join(__dirname, `${directory}`), { recursive: true });
}
...
async function createHTML() {
 ...
 .then(filenames => {
     ...
     for (let filename of filenames) {
         if (filename.isFile() && filename.name.includes('.html')) {
            const readFile = await fsPromises.readFile(path.join(__dirname, 'components', `${filename.name}`));
 ...
 await createDirectory('project-dist');
 createHTML()
 mergeStyles()
